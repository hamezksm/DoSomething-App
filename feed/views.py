...
from .models import Likes, Post, Comments
...
class PostListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            liked=[i for i in Post.objects.all() if Likes.objects.filter(user=self.request.user,post=i)]
...
class UserPostListView(LoginRequiredMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        liked = [i for i in Post.objects.filter(user_name=user) if Likes.objects.filter(user = self.request.user, post=i)]
        context['liked_post'] = liked
        return context
...
def post_detail(request, pk):
	...
	user = request.user
	is_liked =  Likes.objects.filter(user=user, post=post)
